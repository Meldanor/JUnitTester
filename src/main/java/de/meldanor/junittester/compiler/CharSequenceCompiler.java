 ...
 * 
 * @author Kilian Gaertner(modified)
 */
public class CharSequenceCompiler<T> {
    ...
    /**
     * Construct a new instance using this class loader and without any options
     * 
     * @throws IllegalStateException
     *             if the Java compiler cannot be loaded.
     */
    public CharSequenceCompiler() {
        this(CharSequenceCompiler.class.getClassLoader(), null);
    }
    ...
    public Class<T> compileJavaFile(File file) throws FileNotFoundException, ClassCastException, CharSequenceCompilerException {
        ...
        int fileEndingIndex = fileName.indexOf('.');
        if (fileEndingIndex != -1) {
            fileName = fileName.substring(0, fileEndingIndex);
        }

        return compile(fileName, javaSource);
    }
     ...
     *             <var>types</var>.
     */
    public synchronized Class<T> compile(final String className, final CharSequence javaSource, final Class<?>... types) throws CharSequenceCompilerException, ClassCastException {
        this.diagnostics = new DiagnosticCollector<JavaFileObject>();

        Map<String, CharSequence> classes = Collections.singletonMap(className, javaSource);
     ...
     * 
     * @param classes
     *            A Map whose keys are class names and whose values are the Java
     *            source strings containing the definition of the class. A map
     *            value may be null, indicating that compiled class is expected,
     ...
     */
    public synchronized Map<String, Class<T>> compile(final Map<String, CharSequence> classes) throws CharSequenceCompilerException {
        List<JavaFileObject> sources = new ArrayList<JavaFileObject>();
        setQualifiedNames(classes);
    ...
    private void setQualifiedNames(Map<String, CharSequence> classes) {
        Map<String, CharSequence> copy = new HashMap<String, CharSequence>(classes);
        for (Entry<String, CharSequence> entry : copy.entrySet()) {
            String qualifiedClassName = entry.getKey();
            ...
            CharSequence source = entry.getValue();
            String packageName = extractPackageName(source);
            if (packageName != "") {
                classes.remove(qualifiedClassName);
                classes.put(packageName + "." + className, source);
            }
        }
    }

    private static final Pattern PACKAGE_PATTERN = Pattern.compile("package\\s+([a-zA_Z_][\\.\\w]*)\\s*;");

    private String extractPackageName(CharSequence source) {

        Matcher matcher = PACKAGE_PATTERN.matcher(source);
        // has a package
        if (matcher.find()) {
            // Extract the package line
            String s = matcher.group();
            // Delete package, the leading and tailing whitespaces and the
            // finishing semicolon
            return s.replaceAll("package|\\s|;", "");
        }
...
final class JavaFileObjectImpl extends SimpleJavaFileObject {
    ...
    JavaFileObjectImpl(final String baseName, final CharSequence source) {
        super(URI.create(baseName + CharSequenceCompiler.JAVA_EXTENSION), Kind.SOURCE);
        this.source = source;
    }
